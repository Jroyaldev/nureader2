...
export const AnnotationPanel: React.FC<AnnotationPanelProps> = ({
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-[rgba(var(--muted),0.1)] dark:bg-[rgba(255,255,255,0.05)] flex items-center justify-center backdrop-blur-sm shadow-md">
                  <PencilIcon className="w-5 h-5 text-[rgb(var(--accent))]" />
                </div>
                <div>
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Annotations</h2>
                  <p className="text-xs text-gray-600 dark:text-gray-400">({processedAnnotations.length} items)</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            {processedAnnotations.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                <div className="w-16 h-16 rounded-2xl bg-[rgba(var(--muted),0.08)] flex items-center justify-center mb-4">
                  <PencilIcon className="w-8 h-8 text-[rgb(var(--muted))]" />
                </div>
              ...
              <div className="space-y-4">
                {Object.entries(groupedAnnotations).map(([chapter, chapterAnnotations]) => (
                  <div key={chapter} className="space-y-3">
                    <h3 className="text-sm font-semibold text-muted sticky top-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm py-2 -mx-2 px-2 rounded">
                      {chapter}
                    </h3>
                    {chapterAnnotations.map((annotation) => (
                      <div
                        key={annotation.id}
                        className="p-4 rounded-xl bg-white/50 dark:bg-black/50 backdrop-blur-sm border border-white/20 dark:border-gray-700/20 hover:bg-white/70 dark:hover:bg-black/70 transition-all cursor-pointer touch-manipulation"
                        onClick={() => onAnnotationClick(annotation)}
                      >
                        <div className="flex items-start gap-3">
                          <div className={`w-1 h-full rounded-full flex-shrink-0`} style={{ backgroundColor: annotation.color }} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-foreground line-clamp-3 mb-2 font-inter">
                              &ldquo;{annotation.text}&rdquo;
                            </p>
                            {annotation.note && (
                              <p className="text-xs text-muted mb-2 font-inter">
                                {annotation.note}
                              </p>
                            )}
  ...
  return (
    <div className={`fixed right-0 top-0 h-full z-40 transition-all duration-300 ${
      ...
      <div className="unified-panel reader-floating h-full w-[380px] flex flex-col font-inter">
        {/* Header */}
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded-full text-xs transition-all touch-manipulation font-inter ${
                    selectedTags.includes(tag)
                      ? 'bg-[#87a96b]/20 text-[#87a96b] border border-[#87a96b]/30'
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          {annotation.type === 'bookmark' && (
                            <BookmarkSolidIcon className="w-4 h-4 text-[#87a96b]" />
                          )}
